  ...
  chrome.storage.local.get(null, function(data) {
    chrome.system.network.getNetworkInterfaces(function(interfaces) {
      ...
      $('#save').click(function(e) {
        ...
        if (reset) {
          reset = parseFloat($('#resetinterval').val());
          if (!reset) reset = 0;
          if (reset <= 0) {
            reset = false;
        ...
        if (error.length) {
          ...
          if (shownav) chrome.storage.local.set({
            'shownav': shownav
          });
          else chrome.storage.local.remove('shownav');
          if (allowprint) chrome.storage.local.set({
            'allowprint': allowprint
          });
          else chrome.storage.local.remove('allowprint');
          if (hidegslidescontrols) chrome.storage.local.set({
            'hidegslidescontrols': hidegslidescontrols
          });
          else chrome.storage.local.remove('hidegslidescontrols');
          if (local) chrome.storage.local.set({
            'local': local
          });
          else chrome.storage.local.remove('local');
          if (remote) chrome.storage.local.set({
            'remote': remote
          });
          ...
          if (reset) chrome.storage.local.set({
            'reset': reset
          });
          else chrome.storage.local.remove('reset');
          if (useScreensaver && screensaverTime && screensaverURL) {
            chrome.storage.local.set({
          ...
          if (resetcookies) chrome.storage.local.set({
            'clearcookiesreset': resetcookies
          });
          ...
          if (resetcache) chrome.storage.local.set({
          ...
          chrome.storage.local.set({
            'multipleurlmode': multipleurlmode
          });
          if (rotaterate) chrome.storage.local.set({
